import { useEffect, useState } from "react";
import TodoItem from "./TodoItem";


type Priority="Urigente"|"Moyenne"|"Basse"

type Todo={
  id: number;
  text:string;
  priority:Priority
}


function App(){
  const[input,setInput]=useState<string>("")
  const[priority,setPriority]= useState<Priority>("Moyenne")
 

  const savedTodos=localStorage.getItem("todos")
  const  initialTodos=savedTodos?JSON.parse(savedTodos):[]
  const [todos,setTodos]=useState<Todo[]>(initialTodos)

  const [filtere,setFilter]=useState<Priority |"Tous">("Tous")

  useEffect(() => {
    localStorage.setItem("todos",JSON.stringify(todos))
  },[todos])

  function addTodo(){
    if(input.trim()==""){
       return
    }

    const newTodo:Todo={
      id:Date.now(),
      text:input.trim(),
      priority:priority
    }

    const newtodos=[newTodo,...todos]
    setTodos(newtodos)
    setInput("")
    setPriority("Moyenne")
    console.log(newtodos)
     
  }

  
  let filteredTodos: Todo[] = [];

    if (filtere === "Tous") {
    filteredTodos = todos;
  } else {
    filteredTodos = todos.filter((todo) => todo.priority === filtere);
  }

  const urigentecount1= todos.filter((todo)=>todo.priority==="Urigente").length
  const moyennecount1= todos.filter((todo)=>todo.priority==="Moyenne").length
  const bassecount1= todos.filter((todo)=>todo.priority==="Basse").length
  const totalcount1=todos.length

  function deletetodo(id:number){
    const newtodos=todos.filter((todo)=>todo.id !==id)
    setTodos(newtodos)
  }

  const [selectedTodo, setSelectedTodo] = useState<Todo<number>>(new Set())
  return(
    <div className="flex justify-cente">
      <div className="w-2/3 flex-col gap-4 my-15 bg-base-300 p-5 rounded-2xl">
        <div className="flex gap-4">
          <input 
          type="text"
          className="input w-full"
          placeholder="ajoute une tache"
          value={input}
         onChange={(e)=>setInput(e.target.value)}
          />

          <select 
          className="select w-full"
          value={priority}
          onChange={(e)=>setPriority(e.target.value as Priority)}
          >
            <option value="Urigente">Urigente</option>
            <option value="Moyenne">Moyenne</option>
            <option value="Basse">Basse</option>
          </select>

          <button onClick={addTodo} className="btn btn-primary">Ajouter</button>
        </div>
        <div className="space-y-2 flex-1 h-fit">
          <div className="flex flex-wrap gap-4">
            <button
              className={`btn btn-soft ${filtere=== "Tous" ? "btn-primary" :""}`}
              onClick={()=> setFilter("Tous")}
            >
              Tous ({totalcount1})
            </button>

            <button
              className={`btn btn-soft ${filtere=== "urigente" ? "btn-primary" :""}`}
              onClick={()=> setFilter("Urigente")}
            >
              Urigente ({urigentecount1})
            </button>

            <button
              className={`btn btn-soft ${filtere=== "moyenne" ? "btn-primary" :""}`}
              onClick={()=> setFilter("Moyenne")}
            >
              Moyenne ({moyennecount1})
            </button>

            <button
              className={`btn btn-soft ${filtere=== "basse" ? "btn-primary" :""}`}
              onClick={()=> setFilter("Basse")}
            >
              Basse ({bassecount1})
            </button>

            

          </div>
              {filteredTodos.length > 0 ?(
                <ul className="divide-y divide-primary-20">
                  {filteredTodos.map((todo)=>(
                    <li key={todo.id}>
                      <TodoItem 
                      
                      todo={todo}
                      isselelcted={selectedTodo.has(todo.id)}
                      onDelete={()=>deletetodo(todo.id)} 
                      />
                    </li>
                  ))}
                </ul>
              ):(
                <div>Aucune donnee</div>
              )}
        </div>
      </div>
    </div>
  )
}

export default App

